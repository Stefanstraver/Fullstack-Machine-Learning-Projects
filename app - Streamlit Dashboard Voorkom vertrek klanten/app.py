...
# Titel
st.title('Voorspel Klanten Die Opzeggen')
st.write('Druk op voorspel, om te ontdekken of jouw klant waarschijnlijk zal opzeggen')
st.markdown('---')
...
# Sidebar
st.sidebar.info('Verandere deze waardes om te voorspellen!')

# verzamel input features
options={}
...
if st.button('Voorspel'):
    ...
    if prediction[0] == 0:
        st.write("**De klant blijft**")
    else:
        st.write("**De klant gaat weg**")
    with st.expander("Klant details in .json formaat"):
        st.write(options)

st.markdown('---')

# Sla historische data op
try:
...
st.header('Historische resultaten')
if os.path.isfile('historical_data.csv'):
    hist = pd.read_csv('historical_data.csv')
    st.dataframe(hist)
    fig, ax = plt.subplots()
    sns.countplot(x='prediction', data=hist, ax=ax).set_title('Historische voorspellingen')
    st.pyplot(fig)
else:
    st.write('Geen historische data')